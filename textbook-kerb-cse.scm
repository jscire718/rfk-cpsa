...
(herald textbook-kerb-cse (bound 20) (limit 8000))

(defprotocol textbook-kerb-cse basic
...
; ********** Textbook Kerberos Standard Protocol - Authentication Service Exchange (ASE) **********
	...
	(defrole user-ase
		...
		(trace
			...
			(recv (cat (enc tgs n-usr-ase ut-sk (ltk usr as)) tgt))
	...
	(defrole as
		...
		(trace
			...
			(send (cat (enc tgs n-usr-ase ut-sk (ltk usr as)) 
...
; ********** Textbook Kerberos Standard Protocol - Ticket-Granting Service Exchange (TGSE) **********
	...
	(defrole user-tgse
		...
		(trace
			...
			(recv (cat (enc srv n-usr-tgse us-sk ut-sk) st))
	...
	(defrole tgs
		...
		(trace
			...
			(recv (cat srv n-usr-tgse (enc usr time-usr-tgse ut-sk) 
				(enc usr ut-sk (ltk tgs as)) ) )
			(send (cat (enc srv n-usr-tgse us-sk ut-sk) 
